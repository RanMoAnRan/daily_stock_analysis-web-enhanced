...
try:
    ...
    # 允许在未安装 python-dotenv 时启动（例如只跑本地 WebUI）
    ...
    def load_dotenv(dotenv_path: Path, override: bool = False, **_kwargs) -> bool:  # type: ignore
        """python-dotenv 的最小替代：把 .env 键值写入 os.environ。"""
        values = _parse_env_file(Path(dotenv_path))
        for k, v in values.items():
            if override or k not in os.environ:
                os.environ[k] = v
        return True

    def dotenv_values(dotenv_path: Path, **_kwargs) -> dict:  # type: ignore
        """python-dotenv 的最小替代：返回 .env 内的键值对。"""
