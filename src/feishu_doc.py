...
try:
    ...
    FEISHU_DOC_SDK_AVAILABLE = True
except ImportError:  # pragma: no cover
    lark = None  # type: ignore
    FEISHU_DOC_SDK_AVAILABLE = False
...
class FeishuDocManager:
    ...
    def is_configured(self) -> bool:
        """检查配置是否完整"""
        # 只有在 SDK 可用时才认为“可用/可配置”，避免未安装依赖导致启动失败或误判
        return bool(
            FEISHU_DOC_SDK_AVAILABLE
            and self.app_id
            and self.app_secret
            and self.folder_token
        )
    ...
    def create_daily_doc(self, title: str, content_md: str) -> Optional[str]:
        ...
        创建日报文档
        """
        if not FEISHU_DOC_SDK_AVAILABLE:
            logger.warning("未安装飞书 SDK（lark-oapi），跳过创建飞书云文档")
            logger.warning("如需启用请安装：pip install lark-oapi")
            return None

        if not self.client or not self.is_configured():
            logger.warning("飞书 SDK 未初始化或配置缺失，跳过创建飞书云文档")
            return None
