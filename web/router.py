...
# 路由处理函数类型: (request_handler, params) -> Response
RouteHandler = Callable[["BaseHTTPRequestHandler", Dict[str, list]], Response]
...
class Router:
    ...
    def dispatch(
        ...
        try:
            # 调用处理器
            response = route.handler(request_handler, query)
    ...
    def dispatch_post(
        ...
        try:
            # 调用处理器（传入 form_data）
            response = route.handler(request_handler, form_data)
            response.send(request_handler)
...
def create_default_router() -> Router:
    ...
    router.register(
        "/", "GET",
        lambda h, q: page_handler.handle_index(h),
        "配置首页"
    )
    
    router.register(
        "/update", "POST",
        lambda h, form: page_handler.handle_update(h, form),
        "更新配置"
    )

    router.register(
        "/common/update", "POST",
        lambda h, form: page_handler.handle_common_update(h, form),
        "更新常用配置"
    )

    router.register(
        "/env", "GET",
        lambda h, q: page_handler.handle_env_editor(h),
        ".env 配置编辑"
    )

    router.register(
        "/env/update", "POST",
        lambda h, form: page_handler.handle_env_update(h, form),
        "保存 .env 配置"
    )
    ...
    router.register(
        "/health", "GET",
        lambda h, q: api_handler.handle_health(h),
        "健康检查"
    )
    
    router.register(
        "/analysis", "GET",
        lambda h, q: api_handler.handle_analysis(h, q),
        "触发股票分析"
    )
    
    router.register(
        "/tasks", "GET",
        lambda h, q: api_handler.handle_tasks(h, q),
        "查询任务列表"
    )
    
    router.register(
        "/task", "GET",
        lambda h, q: api_handler.handle_task_status(h, q),
        "查询任务状态"
    )

    router.register(
        "/market-review", "GET",
        lambda h, q: api_handler.handle_market_review(h, q),
        "触发大盘复盘"
    )
    ...
    router.register(
        "/bot/feishu", "POST",
        lambda h, form: JsonResponse({"error": "Use POST with JSON body"}),
        "飞书机器人 Webhook"
    )
